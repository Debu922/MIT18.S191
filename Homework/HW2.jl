...
One for every row, so according to the picture 342.
"""
...
function greedy_seam(energies, starting_pixel::Int)
	...
	for i in 2:length(seam)
		x = seam[i-1]
		if x == 1
			_,seam[i] = findmin( energies[i,1:2])
		elseif x == n
			_,seam[i] = findmin( energies[i,end-1:end])
			seam[i] += n-2
		else
			_,seam[i] =findmin( energies[i,x-1:x+1])
			seam[i] += x-2
		end
	end
	return seam
...
if shrink_greedy
	greedy_carved = shrink_n(img, 400, greedy_seam)
	md"Shrink by: $(@bind greedy_n Slider(1:400; show_value=true))"
end
