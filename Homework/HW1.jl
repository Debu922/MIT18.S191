...
# ╔═╡ 40881910-4865-11eb-1ff7-3798bccfbc7b
typeof([[1,2], [3,4]])
...
# ╔═╡ ab4fc2c0-4865-11eb-0a6c-db550806559a
...
# ╔═╡ 1fc7dd0e-4869-11eb-1761-552ef1069515
color = RGB(0.2,0.45,0.13)

# ╔═╡ ce65f880-4868-11eb-288c-ad505b2bacf8
typeof(RGB(quantize(color.r),quantize(color.g),quantize(color.b)))
...
# ╔═╡ 70ac1b40-486b-11eb-2b38-1920b4dcf32b
...
function blur_1D(v, l)
	...
	# for i in 1:length(v)
	# 	v_blur[i] = mean([extend(v,i+j) for j in -l:l])
	# end
	# return v_blur
end
...
function convolve_vector(v, k)
	...
	l = (length(k)-1)÷2
	ev = [extend(v, i) for i in (1-l):(s+l)]
	return [sum(ev[i:(i+2*l)].*k) for i in 1:s]
end
...
function gaussian_kernel(n)
	...
	a = [g(i) for i in -(n÷2):n÷2]
	return a./sum(a)
end

# ╔═╡ 664b6270-48e1-11eb-1c33-3b4d2fa7d90d
gaussian_kernel(7)

# ╔═╡ bae5f940-48e0-11eb-2762-a38b8a33bf0d
colored_line(gaussian_kernel(7))

# ╔═╡ f8bd22b8-ee14-11ea-04aa-ab16fd01826e
md"Let's test your kernel function!"

# ╔═╡ 576f53c0-48e0-11eb-0d9e-d9e5f1e794eb
@bind kernal_size Slider(1:2:50, show_value=true)

# ╔═╡ 2a9dd06a-ee13-11ea-3f84-67bb309c77a8
gaussian_kernel_size_1D = kernal_size # change this value, or turn me into a slider!
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	nc_K, nr_K = size(K)
	nc_M, nr_M = size(M)
	M_pad = [extend_mat(M,i,j) for (i,j) in Iterators.product(1-nc_K÷2:nc_M+nc_K÷2,1-nr_K÷2:nr_M+nr_K÷2)]
	return [sum(M_pad[i:i+nc_K-1,j:j+nr_K-1].*K) for (i,j) in Iterators.product(1:nc_M, 1:nr_M)]
end
...
# ╔═╡ 09e8fde0-48ef-11eb-2dfa-11cad1f2ea84
M = test_image_with_border

# ╔═╡ b3b85330-48ee-11eb-349a-cd5bc8f200ad
begin
	K = K_test
	nc_K, nr_K = size(K)
	nc_M, nr_M = size(M)
	M_pad = [extend_mat(M,i,j) for (i,j) in Iterators.product(1-nc_K÷2:nc_M+nc_K÷2,1-nr_K÷2:nr_M+nr_K÷2)]
	[sum(M_pad[i:i+nc_K -1,j:j+nr_K -1]*K) for (i,j) in Iterators.product(1:nc_M, 1:nr_M)]
end
...
# ╔═╡ e918f550-48f0-11eb-0240-9bdc1b3ad4a6
function normalize(K)
	return K./sum(K)
end

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image, n=7)
	g(x,y) = exp(-(x^2+y^2)/2)/(2π)
	
	K = normalize([g(i-(n+1)/2,j-(n+1)/2) for (i,j) in Iterators.product(1:n,1:n)])
	# @show K
...
# ╔═╡ d4a85820-48f2-11eb-1040-99b046126b5e
K, img=with_gaussian_blur(philip,7)

# ╔═╡ b41383d0-48f4-11eb-2c7f-cfa803500f77
K
...
function with_sobel_edge_detect(image)
	...
	K_1 = a'*b
	K_2 = b'*a
	G_x = convolve_image(image,K_1)
	G_y = convolve_image(image,K_2)
	G_t = (G_x.*G_x + G_y.*G_y).^.5
	@show size(K_1)
	return G_t
end
