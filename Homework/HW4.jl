...
student = (name = "Debaditya Bhattacharya", kerberos_id = "debbh")
...
The persion is guarenteed to contract the illness if exposed.
"""
...
large_experiment = do_experiment(0.25, 10000) 
...
# ╔═╡ 37abf68a-4d25-11eb-2553-d742dadfcbf1
function sin_cos_plot()
    T = -1.0:0.01:π
...
# ╔═╡ 3a72b9bc-4d25-11eb-0c31-5b2cfce437d8
sin_cos_plot()
...
# ╔═╡ a1220c4e-4d25-11eb-25d2-7bed25480987
...
# ╔═╡ 72614d7e-4d26-11eb-2986-2b3730230694
@bind p_new Slider(0.01:0.01:1, show_value=true)

# ╔═╡ 71a1afdc-4d26-11eb-1440-7f0a729912de
@bind N_new Slider(1:100_000, show_value=true)

# ╔═╡ bb63f3cc-042f-11eb-04ff-a128aec3c378
begin
	data_new = do_experiment(p_new,N_new)
	frequencies_plot_with_mean(data_new)
end
...
# ╔═╡ 9d045ff6-4d28-11eb-2389-e1394a63cc51
@bind c Slider(-5:0.1:5)

# ╔═╡ 0024f916-4d28-11eb-2b78-9b6bd81368e5
@bind t_exp Slider(0.1:.1:10)

# ╔═╡ 7bb8e426-0495-11eb-3a8b-cbbab61a1631
begin
	plot_new = frequencies_plot_with_mean(do_experiment(p_new,N_new))
	plot!(plot_new,[c*exp(-x*t_exp) for x = 1:maximum(data_new)])
end
...
begin
	...
	mean_time = [mean(do_experiment(p,10000)) for p = 0.001:0.001:1]
	typeof(mean_time)
	plot(mean_time,0.001:0.001:1)
end
...
# ╔═╡ 60654c3e-4d2a-11eb-261e-0186989c2995
typeof(test_status)
...
function generate_agents(N::Integer)
	agent_vector = [Agent() for i in 1:N]
	set_status!(rand(agent_vector),I)
	return agent_vector
end
...
# ╔═╡ f0f030ca-4d2f-11eb-0d48-ed14bb86b6c5
function count_status(agents::Vector{Agent},status::InfectionStatus)
	sum(status==agent.status for agent in agents)
end
...
# ╔═╡ 745c1796-4dc6-11eb-28bb-b17c5123bb53
...
begin
	function interact!(agent::Agent, source::Agent, infection::InfectionRecovery)
		if is_infected(agent)
			...
			return
		elseif is_infected(source) && is_susceptible(agent) 
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	for i in 1:T
		...
		S_counts[i] = count_status(agents,S)
		I_counts[i] = count_status(agents,I)
		R_counts[i] = count_status(agents,R)
...
let
	...
	result = plot(1:T, sim.S, ylim=(0, N), label="Susceptible",xlabel="No. Of Steps", ylabel="No. of Agents"
	)
...
# ╔═╡ cd45f814-4de6-11eb-2428-315361b7d64d
let
	N = 1000 
	T = 10000
	agents = generate_agents(N)
	infection = InfectionRecovery(0.2,0.02)
	for i = 1:T
		sweep!(agents,infection)
	end
...
simulations_2 = repeat_simulations(100, 1000, Reinfection(p_infection, p_recovery), 20)

# ╔═╡ 694dc9aa-4def-11eb-123a-3db03d53576a
@bind n_plot Slider(1:length(simulations_2),show_value = true)

# ╔═╡ f9202730-4ded-11eb-3ecd-b57bb3f0b39d
let
	p = plot()
	
	for i in 1:n_plot
		plot!(p, 1:1000, simulations_2[i].I, alpha=.5, label=nothing)
	end
	
	p
end

# ╔═╡ a1f04080-4def-11eb-009c-f17d4b2868a0
plot(1:1000, simulations_2[n_plot].I, alpha=.5, label=nothing)
